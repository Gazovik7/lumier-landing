...
/* ===================================================================
   ...
   2) TELEGRAM — основной канал, отправка из браузера прямо в Bot API.
      chatId — id чата/группы, лежит здесь открыто (сам по себе бесполезен).
               У групп ОТРИЦАТЕЛЬНЫЙ, без минуса Telegram отвечает 400.
      token  — НЕ хранится в репозитории. Приезжает из js/lead-secrets.js,
               куда его подставляют при публикации из секрета TG_BOT_TOKEN
               (.github/workflows/pages.yml). Репозиторий публичный, а по
               GitHub ходят сканеры, которые ищут именно такие токены.
      ЧЕГО ЭТО НЕ РЕШАЕТ: заявку отправляет браузер, поэтому на живой странице
      токен всё равно виден через «просмотр кода». Посторонний может спамить
      в чат и выгнать бота из группы (leaveChat) — то есть тихо оборвать канал.
      Если это станет проблемой — см. пункт 3, там токен не покидает сервер.

   3) WEBHOOK — свой обработчик, получает JSON вида
      { source, name, phone, ... , page, ts }.
      В lead-proxy/ лежит готовая функция для Yandex Cloud Functions: она
      держит токен у себя и сама пишет в группу заявок, в браузер токен не
      попадает совсем. Это более надёжный вариант вместо пункта 2, но нужен
      аккаунт Yandex Cloud. Разворачивание — lead-proxy/README.md.
      Включать одновременно с пунктом 2 не нужно: заявка придёт дважды.
   =================================================================== */
var LEAD_CONFIG = {
  email: {
    endpoints: [
  ...
  // Токен НЕ хранится в репозитории: он приходит из js/lead-secrets.js, куда его
  // подставляют при публикации из секрета TG_BOT_TOKEN (.github/workflows/pages.yml).
  ...
  telegram: { token: window.LEAD_TOKEN || "", chatId: "-5163834689" },
